...
export const validateAddress = [
  ...
  body('pin')
    ...
    .matches(/^[1-9][0-9]{5}$/)
    .withMessage('PIN code must be a valid 6-digit Indian postal code'),
  ...
  body('altPhone')
    ...
    .matches(/^[6-9]\d{9}$/)
    .withMessage('Alternate phone must be a valid 10-digit Indian number')
    .custom((value, { req }) => {
      if (value && value === req.body.phone) {
        throw new Error(
          'Alternate phone cannot be the same as the primary phone',
        );
      }
      return true;
    }),
  ...
  body('defaultAddress')
    .optional()
    .isBoolean()
    .withMessage('Default address must be true or false'),
];
