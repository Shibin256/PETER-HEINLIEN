...
export const changeOrAddMobile = async (req, res) => {
  try {
    ...
    if (phoneExist && phoneExist.phone != null) {
      return res
        .status(400)
        .json({ data: user, message: 'The mobile number is already exists' });
    }
  ...
  try {
    ...
    if (currentPassword === newPassword) {
      return res
        ...
        .json({
          message:
            'You entered the same password. Please choose a different one.',
        });
    }
...
export const addAddress = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: 'address created', user: user, address: newAddress });
  } catch (error) {
...
export const removeAddress = async (req, res) => {
  try {
    ...
    if (!addressExist) {
      return res
        .status(400)
        .json({ message: 'Address does not belong to this user' });
    }
...
export const SetDefaultAddress = async (req, res) => {
  try {
    ...
    await Address.findByIdAndUpdate(addressId, { defaultAddress: true }).select(
      '-createdAt -updatedAt',
    );
