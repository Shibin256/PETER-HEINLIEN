...
export const createCoupons = async (req, res) => {
  try {
    ...
    const existingCoupon = await Coupons.findOne({ code: couponCode }).select(
...
export const fetchCoupons = async (req, res) => {
  ...
  let coupons = await Coupons.find(filter)
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit)
...
export const deleteCoupon = async (req, res) => {
  ...
  try {
    const deletedCoupon = await Coupons.findByIdAndDelete(couponId).select(
      '-createdAt -updatedAt',
    );
    if (!deletedCoupon) {
...
export const applyCoupon = async (req, res) => {
  ...
  try {
    const coupon = await Coupons.findOne({ code: couponCode }).select(
      '-createdAt -updatedAt',
    );
...
export const removeCoupon = async (req, res) => {
  ...
  try {
    const coupon = await Coupons.findOne({ code: couponId }).select(
      '-createdAt -updatedAt',
    );
    if (!coupon) {
      return res.status(404).json({ message: 'Coupon not found' });
    ...
    res
      .status(200)
      .json({ message: 'User removed from coupon successfully', coupon });
  } catch (error) {
    console.error('Error removing user from coupon:', error);
    res
      .status(500)
      .json({ message: 'Server error while removing user from coupon' });
  }
};
