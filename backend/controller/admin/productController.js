...
export const createProduct = async (req, res) => {
  try {
    let available = false;
    const { name, description, category, brand, tags, price, quantity } =
      req.body;
    const uploadImage = [];
    if (req.files.length < 3) {
      return res
        .status(404)
        .json({ message: 'The total of 3 images needed to proceed' });
    }
...
export const getTopRatedProduct = async (req, res) => {
  try {
    const { userId } = req.params;
    const topRatedCollections = await Product.find({ isList: { $ne: true } })
...
export const getBrandsAndCollection = async (req, res) => {
  try {
    const category = await Category.find({ isList: { $ne: true } }).select(
      '-createdAt -updatedAt',
    );
    const brands = await Brands.find({ isList: { $ne: true } })
      .sort({ name: 1 })
      .select('-createdAt -updatedAt');

    const result = await Product.find({ isList: { $ne: true } }).select(
      '-createdAt -updatedAt',
    );
  ...
  try {
    ...
    if (product.offerPrice) {
      if (product.offerPrice > product.price - discountAmount) {
        ...
        return res
          .status(404)
          .json({ message: 'The Category offer is heigher than this offer' });
      }
    } else {
      product.offerPercentage = percentage;
      product.offerPrice = product.price - discountAmount;
