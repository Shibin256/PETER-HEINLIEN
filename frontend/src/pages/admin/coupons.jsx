...
import { createCoupons, deleteCoupon, fetchCoupons, updateCoupon } from '../../features/coupons/couponsSlice';
...
const Coupons = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (couponCode == '' || discountType == '' || discountAmount == '' || discountAmount == '' || minPurchase == '' || usageLimit == '' || expirationDate == '') {
    ...
    const { loading, coupons, page, totalPages } = useSelector(state => state.coupons)
    ...
    const handleUpdateCoupon = async (e) => {
        ...
        if(editingCoupon.code === '' || editingCoupon.discountType === '' || editingCoupon.discountValue === '' || editingCoupon.minPurchase === '' || editingCoupon.usageLimit === '' || editingCoupon.expiresAt === '') {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
                <h3 className="text-xl font-semibold text-gray-800 mb-4">Create New Coupon</h3>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4">
                    <h3 className="text-xl font-semibold text-gray-800 mb-4 md:mb-0">Active Coupons</h3>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {coupons.map((coupon) => (
                                <tr key={coupon._id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{coupon.discountType === 'fixed' ? `₹ ${coupon.discountValue}.00 off` : `${coupon.discountValue} % `}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{`${coupon.usersUsed.length}/${coupon.usageLimit}`}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{coupon.expiresAt.split("T")[0]}</td>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mt-4 text-sm text-gray-600">
                    ...
                    <div className="flex justify-center items-center gap-4 mt-6">
                        <button
                            disabled={page <= 1}
                            onClick={() => dispatch(fetchCoupons({ page: page - 1, limit: 4, search: searchTerm }))}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold text-gray-800">Edit Coupon</h3>
                            <button 
                        ...
                        <form onSubmit={handleUpdateCoupon}>
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        {editingCoupon.discountType === 'fixed' ? 'Discount Amount (₹)' : 'Discount Percentage (%)'}
                                    </label>
                                    <div className="relative">
                                        <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">
                                        ...
                                        <input
                                            ...
                                            step={editingCoupon.discountType === 'fixed' ? "0.01" : "1"}
                                            min="0"
                                            max={editingCoupon.discountType === 'percentage' ? "100" : ""}
                                            className="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        />
                                    </div>
                                </div>
