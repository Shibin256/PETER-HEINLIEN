...
const ProductAdmin = () => {
  ...
  // State for list/unlist confirmation
  const [showListConfirm, setShowListConfirm] = useState(false);
  const [productToList, setProductToList] = useState(null);
  const [listAction, setListAction] = useState(null); // 'list' or 'unlist'
  ...
  const listedCount = products?.filter((product) => !product.isList).length || 0;
  const unlistedCount = products?.filter((product) => product.isList).length || 0;
  ...
  const handleListToggle = (product, action) => {
    setProductToList(product);
    setListAction(action);
    setShowListConfirm(true);
  };

  const confirmListToggle = async () => {
    if (!productToList) return;
    ...
    try {
      ...
      setProductToList(null);
      setListAction(null);
    } catch (error) {
      toast.error(error?.message || `Failed to ${listAction} product.`);
      setShowListConfirm(false);
      setProductToList(null);
      setListAction(null);
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-col md:flex-row items-end md:items-center gap-4 w-full md:w-auto">
          <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-purple-100 text-purple-800 px-3 py-1 rounded-lg shadow-sm border border-purple-200 text-sm">
              Listed: <span className="font-bold">{listedCount}</span>
            </div>
            <div className="bg-orange-100 text-orange-800 px-3 py-1 rounded-lg shadow-sm border border-orange-200 text-sm">
              Unlisted: <span className="font-bold">{unlistedCount}</span>
            </div>
          </div>
        </div>
      </div>
