...
const Wishlist = () => {
    ...
    const handleAddAllToCart = async (e) => {
        ...
        try {
            ...
            const res = await dispatch(wishlistToCart({ userId: user._id, productIds: productIds }))
            console.log(res)
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-12">Select</th>
                                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[200px]">Product</th>
                                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Price</th>
                                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Availability</th>
                                <th className="px-6 py-4 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-20">Action</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {wishlist?.map((item) => (
                                <tr key={item._id} className="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${item.stockStatus == 'In Stock' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                            }`}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right">
                                        <button
                                            ...
                                            className={`inline-flex items-center justify-center p-1 rounded-md ${isDeletingId === item._id
                    ...
                    <div className="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div className="flex items-center space-x-4 w-full sm:w-auto">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium text-white ${!selectedAction || isProcessing
                        ...
                        <button
                            ...
                            className={`px-6 py-2 rounded-md text-sm font-medium text-white ${wishlist?.length === 0 || isProcessing
                                ? 'bg-gray-400 cursor-not-allowed'
