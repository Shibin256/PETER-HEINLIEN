...
const OTPFormFrogotpass = () => {
  ...
  const handleResend=async()=>{
    setLoading(true)
      try {
        console.log()
        const response = await axiosInstance.post(`${baseUrl}/api/auth/forgotpass`, formData);
        if(response){
...
//time format
  ...
  const handleSubmit = async() => {
    ...
    const response=await axiosInstance.post(`${baseUrl}/api/auth/verifyOTPForgotpass`,{formData,otp})
