...
const getSalesReport = async ({ type, startDate, endDate }) => {
    ...
    const res = await adminAxiosInstance.get("/api/admin/dashboard/sales-report", {
...
const getRecentOrders = async () => {
    const res = await adminAxiosInstance.get("/api/admin/dashboard/recent-orders");
    return res.data;
};
...
const downloadSalesReportPdf = async ({ type, startDate, endDate }) => {
    console.log(type, startDate)
    const res = await adminAxiosInstance.get("/api/admin/dashboard/sales-report/pdf", {
        params: { type, startDate, endDate },
        responseType: 'blob'
    });
...
const downloadSalesReportExcel = async ({ type, startDate, endDate }) => {
    const res = await adminAxiosInstance.get("/api/admin/dashboard/sales-report/excel", {
        params: { type, startDate, endDate },
        responseType: 'blob'
    });
